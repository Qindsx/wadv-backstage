...
import { grossIndicesgrossDrawer } from "@/components/drawer";
...
async function getData(size = 10, page = 1) {
  ...
  loading.value = true;
  const res = await getProductionValueInfo({
    year: year.value,
    limit: size,
    offset: size * (page - 1)
  });
...
async function deleteData(years: string[]) {
  const res = await deleteProductionValue({ year: years });
  if (res.message) {
...
const exportExcel = async () => {
  const resData = await getProductionValueInfo({ year: [] });
  if (resData.data) {
    ...
    writeFile(workBook, "农业生产条件和农业投入数据报表.xlsx");
...
const handleMany = async () => {
  ...
  // console.log(list);
  const res = await addProductionValue({ data: list });
  if (res.message) {
    ElMessage.success(res.message);
    getData();
  }
  ...
  <div>
    ...
    <el-card>
      ...
      <div class="pb-2 flex">
        ...
        <el-link
          type="primary"
          href="/xlsxTemplate/农业生产条件和农业投入数据模板.xlsx"
          >下载模板</el-link
        >
      </div>
      <pure-table
